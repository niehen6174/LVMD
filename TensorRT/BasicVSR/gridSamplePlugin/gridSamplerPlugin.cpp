    ...
    const void* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream)
{
    ...
    status = grid_sampler_2d_cuda(mBatch, inputs[0], inputs[1], outputs[0],
        ...
        mGridHeight*mGridWidth*2, mGridWidth*mGridHeight, mGridWidth, 1,
    ...
    const DynamicPluginTensorDesc* inputs, int nbInputs, const DynamicPluginTensorDesc* outputs, int nbOutputs)
{
    ...
    mGridHeight = inputs[1].desc.dims.d[2];
    mGridWidth = inputs[1].desc.dims.d[3];
    mType = inputs[0].desc.type;

    ASSERT(mBatch == inputs[1].desc.dims.d[0]);
    ASSERT(inputs[1].desc.dims.d[1] == 2); // only supports coor = 2
}
