...
const char* INPUT_BLOB_NAME2 = "data2";
...
ICudaEngine* createNetEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt)
{
    ...
    ITensor* data2 = network->addInput(INPUT_BLOB_NAME2, dt, Dims3{1, INPUT_H, INPUT_W});
    ...
    std::vector<ITensor*> input_data = {data, data2};
    
    //这个name可以随便设置 不影响。
    IPluginV2 *addscalar_plugin = addscalar_creator->createPlugin("addplugin", &fc);
    IPluginV2Layer* addscalar_layer = network->addPluginV2(input_data.data(), 2, *addscalar_plugin);
...
void doInference(IExecutionContext& context, float* input, float* input2,float* output, int batchSize)
{
    ...
    CUDA_CHECK(cudaMalloc(&buffers[inputIndex2], batchSize * INPUT_H * INPUT_W * sizeof(float)));
    ...
    CUDA_CHECK(cudaMemcpyAsync(buffers[inputIndex2], input2, batchSize * INPUT_H * INPUT_W * sizeof(float), cudaMemcpyHostToDevice, stream));
